...
referrals_freqs = [(k, len(list(g)))
                   for k, g in groupby(sorted(freq_of_referrals))]
pending_eligibility = [(k, len(list(g)))
                       for k, g in groupby(sorted(pending_eligibility_locations))]
...
print 'Number of patients who have completed Tp1 and have completed Treatment Session 1: %s' % first_apt_therapy

print 'Number of patients who have completed Tp1 and have not completed Treatment Session 1: %s' % first_apt_not_sched
...
print ''
print 'Healthy Vets with pending online screens: %s' % hv_pending_screen
print 'Healthy Vets that have completed the online screen: %s' % hv_completed_screen
print 'Healthy Vets in the process of being scheduled for Tp1: %s' % hv_process_tp1
print 'Healthy Vets scheduled for Tp1 (as of Today\'s Date): %s' % hv_tp1_sched
print ''
