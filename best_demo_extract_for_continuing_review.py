...
best_rc_df=best_project.export_records(event_name='unique',events=['tp1_day1_arm_1'],fields=['record_id_redcap'],forms=['participant_reachout_recruitment','best_master','consent','online_screening_survey'],format='df',df_kwargs={'dtype': 'str'})
os_rc_df=os.export_records(event_name='unique',fields=['OnlineID'],forms=['final_determination','demographics_and_background','online_screening_survey'],format='df',df_kwargs={'dtype': 'str'})
...
merged=best_rc_df.merge(os_rc_df.reset_index(),left_on='redcap_id_main',right_on='online_id',how='left',copy=False)
